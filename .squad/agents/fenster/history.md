...
## Learnings
- Admin backend diagnosis (2026): Build passes clean but the admin dashboard is largely non-functional. 4 critical findings: (1) Squads, Moderation, AuditLog pages are stubs — they show "will connect in future update" placeholders despite the API having full admin endpoints for all of them. (2) Home/Dashboard page is static cards with no data — doesn't call GET /api/admin/dashboard which returns network stats. (3) Logout form in MainLayout.razor is missing AntiforgeryToken — POST /logout will fail with 400 because app.UseAntiforgery() is enabled but the form doesn't include a token and the endpoint doesn't call .DisableAntiforgery(). (4) DiscoveryPrompt save (PUT) will fail in production because the Admin HttpClient doesn't send X-Squad-Api-Key header, and ApiKeyMiddleware requires it for write operations outside dev mode.
